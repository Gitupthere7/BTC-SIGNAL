...
# ── LONG conditions (6 required of 6) ─────────────────────────────────
long_checks = [
    ("EMA ribbon stacked bullish (9>21>50)",    e9v > e21v > e50v),
    ("RSI dipped recently then recovered",      (rvp < 55 or rvp2 < 55) and 44 < rvv < 65 and rvv > rvp),
...
# ── SHORT conditions (6 required of 6) ────────────────────────────────
short_checks = [
    ("EMA ribbon stacked bearish (9<21<50)",    e9v < e21v < e50v),
    ("RSI elevated then falling",               (rvp > 45 or rvp2 > 45) and 35 < rvv < 56 and rvv < rvp),
...
# Bonus signals that boost confidence score
long_bonus = [
...
long_all  = long_checks + long_bonus
short_all = short_checks + short_bonus
...
# What's missing (for STAY OUT context)
if signal == "STAY OUT":
    missing_long  = [name for name, v in long_checks  if not v]
    missing_short = [name for name, v in short_checks if not v]
    if long_conf >= short_conf:
        reasons = [f"Missing for LONG: {m}" for m in missing_long[:3]]
    else:
        reasons = [f"Missing for SHORT: {m}" for m in missing_short[:3]]
...
return {
    ...
    "rsi":        round(float(rvv), 1),
    "macd":       round(float(mhv), 2),
    "atr":        round(float(atrv), 0),
...
```
if s == "STAY OUT":
    missing = "\n".join(f"  - {r}" for r in sig["reasons"][:3])
    return (
        f"<b>STAY OUT - BTC/USD</b>\n"
        f"Was: {prev}   Now: STAY OUT\n"
        f"Price: ${p:,.2f}\n"
        f"Regime: {sig['regime']}  RSI: {sig['rsi']}  MACD: {sig['macd']}\n\n"
        f"<b>Not firing because:</b>\n{missing}\n\n"
        f"Long score:  {sig['long_base']}/6 base conditions\n"
        f"Short score: {sig['short_base']}/6 base conditions\n"
        f"All 6 base conditions required to signal.\n\n"
        f"<i>Patience. Wait for the full setup.</i>\n"
        f"{now_utc()}"
    )
...
reasons  = "\n".join(f"  {chr(9658)} {r}" for r in sig["reasons"])

return (
    f"{'<b>LONG - BTC/USD</b>' if s=='LONG' else '<b>SHORT - BTC/USD</b>'}\n"
    f"{'─' * 30}\n"
    f"Price:      ${p:>12,.2f}\n"
    f"Target:     ${tp:>12,.2f}   ({tp_label})\n"
    f"Stop:       ${sl:>12,.2f}   ({sl_label})\n"
    f"Confidence: {sig['confidence']}%\n"
    f"Regime:     {sig['regime']}  EMA200: {sig['e200_slope']}\n"
    f"RSI: {sig['rsi']}   MACD: {sig['macd']}   ATR: {sig['atr']}\n\n"
    f"<b>Why {s}:</b>\n{reasons}\n\n"
    ...
    f"  Max loss:    ${sig['risk_usd']:>10,.2f}  (2% capital)\n"
    f"  Est liq:     ${sig['liq_price']:>10,.2f}  <b>CHECK THIS</b>\n\n"
    f"<b>ACTION: {arrow} on Kraken margin</b>\n\n"
    f"Backtest stats: 84% WR  |  8.9 PF  |  5.4% max DD\n"
    f"{now_utc()}\n"
    f"<i>Signal only. Verify on Kraken 1H chart before executing.</i>"
)
```
...
log(f"Previous signal: {prev or 'None'}")
...
log(f"Signal: {sig['signal']}  Confidence: {sig['confidence']}%  "
    f"Regime: {sig['regime']}  RSI: {sig['rsi']}  MACD: {sig['macd']}")
log(f"Long base: {sig['long_base']}/6  Short base: {sig['short_base']}/6")
...
if sig["signal"] in ("LONG", "SHORT"):
    if sig["confidence"] >= MIN_CONFIDENCE:
        if sig["signal"] != prev:
            should_alert = True
            log(f"Direction change: {prev} -> {sig['signal']}. Alerting.")
        else:
            log("Same direction as last alert. No repeat.")
    else:
        log(f"Confidence {sig['confidence']}% below threshold {MIN_CONFIDENCE}%.")
